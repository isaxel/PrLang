Задача заключается в том, чтобы создать на языке C консольное приложение, которое будет работать как простой HTTP-сервер, который может обслуживать запросы на файлы из определённой директории. Кроме того, нужно использовать tasks, чтобы обрабатывать несколько запросов одновременно. tasks - процессы через fork(), создаем многопроцессный HTTP-сервер: каждый новый клиент → новый процесс.
